...
export default function HistorialAsistenciaPage({ params }: { params: Promise<{ grupoId: string }> }) {
  ...
  async function handleDescargar(format: 'pdf' | 'xlsx') {
    setDownloading(true);
    try {
      const token = await getAccessTokenSilently();
      const url = getReporteAsistenciaUrl(grupoId, from, to, format);
      const res = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
      if (!res.ok) throw new Error('No se pudo descargar el reporte');
      const blob = await res.blob();
      const a = document.createElement('a');
      a.href = URL.createObjectURL(blob);
      a.download = `asistencia-${nombre.replace(/\s+/g, '_')}-${from}-${to}.${format}`;
      a.click();
    } catch (e: unknown) {
      // silencio — el usuario verá que no descargó
      console.error(e);
    } finally {
      ...
      <div className="flex flex-wrap items-end gap-4 mb-6 bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
        ...
        <div className="ml-auto flex gap-2">
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-semibold px-3 py-2 border border-green-300 text-green-700 bg-green-50 hover:bg-green-100 rounded-lg transition-colors disabled:opacity-50"
          >
          ...
          <button
            ...
            disabled={downloading}
            className="flex items-center gap-1.5 text-xs font-semibold px-3 py-2 border border-red-300 text-red-700 bg-red-50 hover:bg-red-100 rounded-lg transition-colors disabled:opacity-50"
          >
